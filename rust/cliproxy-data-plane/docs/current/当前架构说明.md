...
- 为 `/v1/responses` 上游失败产出 RESP `errors` 事件，并在本地维护 auth/model 级运行时健康 overlay
...
flowchart TD
    ...
    AuthState[auth_state.rs]
    ErrorEvents[error_events.rs]
    RedisProtocol[redis_protocol.rs]
    Responses[responses.rs]
    ResponsesHandler[responses/handler.rs]
    ResponsesFailure[responses/failure.rs]
    ResponsesRouting[responses/routing_policy.rs]
    ResponsesHealth[responses/health.rs]
    ...
    Responses --> Telemetry
    Responses --> AuthState
    Responses --> ErrorEvents
    Responses --> RouterCore
    Responses --> UpstreamRuntime
    Responses --> Common
    Responses --> ResponsesHandler
    Responses --> ResponsesFailure
    Responses --> ResponsesRouting
    Responses --> ResponsesHealth
...
除此之外，`/v1/responses` 现在还有一层**进程内运行时 health overlay**：
...
- 不写回 snapshot
- 不做持久化
- 仅用于当前 Rust 进程内的候选过滤、pre-commit retry 和 RESP `errors` 事件
...
`router-core` 产出的仍然是“理想路由计划”，但真正进入上游执行前，Rust 现在还会经过一层本地策略收口：

- `responses/routing_policy.rs`
  - 基于 snapshot 自带 `cooldown_until`
  - 再叠加 Rust 进程内 auth/model health overlay
  - 生成当前时刻真正可执行的 `effective plan`
- `responses/failure.rs`
  - 把上游错误字符串先收口成 typed failure kind
  - 供 pre-commit retry、cooldown 决策和 recorder 共享
- `responses/health.rs`
  - 负责把失败写入 auth/model overlay
  - 同时产出 RESP `errors` 事件

这样 `handler.rs` 保持编排职责，`upstream.rs` 只负责执行与重试，失败语义和冷却策略不再散落在多个调用点。
...
执行入口仍在 `responses/upstream.rs::execute_upstream_with_retries()`，但现在会复用：

- `responses/failure.rs` 做 typed failure classification
- `responses/health.rs` 写入 overlay 和 `errors` 事件
- `responses/routing_policy.rs` 提前过滤已冷却中的候选 auth
...
Go 到 Rust 的控制面通信以 runtime snapshot 为核心：

- Go 是配置、auth 和管理状态的事实来源
- Rust 周期性拉取 snapshot，并按版本原子应用
- Go 可以通过 notify 提醒 Rust 立即 refresh，但 Rust 仍以拉取完整 snapshot 为准

Rust 到 Go 的反馈面目前以 usage 回填为最小闭环：

- Rust 为 `/v1/responses` 产出 CPA-shaped usage payload
- Go bridge 优先通过 RESP 订阅 Rust usage，并回填 CPA `internal/redisqueue`
- 健康状态、auth 健康信号和 cooldown 建议仍未形成完整稳定回传协议

热路径边界保持独立：

- 真正的请求热路径是 `Client -> Rust -> Provider -> Rust -> Client`
- snapshot 拉取、usage 回填、健康探测属于非热路径控制 / 反馈通信
- 本地 Go / Rust 控制面通信应直连，不应走全局出口代理；代理策略只应用于外部 provider 请求
