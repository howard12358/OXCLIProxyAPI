...
- 按 CPA `internal/redisqueue/plugin.go` 形状异步输出 usage payload 日志
...
flowchart TD
    ...
    ResponsesMock[responses/mock.rs]
...
- Rust 进程直接暴露 CPA redis 订阅 / pop 协议
...
2. 解析请求元数据
3. 基于 snapshot 生成 execution plan
...
- Rust 数据平面虽然抽象里保留了 `OpenAi / Codex / Mock`
- 但当前 `/v1/responses` 主链路主要按 Codex 场景设计和验证

### 10.4 当前模块拆分

当前 `responses` 目录按职责拆成四层：
...
- `upstream.rs`
  - 承担真实 upstream 执行、Codex 请求归一化和 auth retry
- `sse.rs`
  - 承担 SSE frame 重组、流尾 flush 和 `response.completed.output` 修复
- `mock.rs`
  - 承担 mock fallback 的 JSON / SSE 回包
