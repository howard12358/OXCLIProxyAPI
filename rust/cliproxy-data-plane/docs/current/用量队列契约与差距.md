...
| payload 形状 | 已定型，见 `queuedUsageDetail` | 基本对齐 `UsageQueuePayload` | `source` 已由 Go snapshot 的 `usage_source` 提供，`api_key` 已在 Rust HTTP 入口从下游请求头提取；`reasoning_effort` 等少数字段仍可能是默认值或缺口。 | 统计主字段和常用归因字段可用；依赖更细粒度字段的报表仍需继续对齐。 |
...
| 错误事件通道 | `errors` channel 单独订阅 | 已支持订阅通道，且 `/v1/responses` 预提交上游失败会产出最小错误 payload | 已补单机最小闭环；字段和覆盖范围仍比 CPA 更窄。 | 当前事件主要服务 Rust 本地 auth/model 健康 overlay 和运维订阅，不是 Go 全量 error event 复刻。 |
...
| errors 通道生产来源 | 已完成单机最小闭环；当前只覆盖 Rust `/v1/responses` 上游失败，不覆盖 Go 全路径 |
